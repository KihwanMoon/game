...
#     발밑이 아니라 겨눈 곳이다 (설계/5_스킬 §10.3). 같은 시드가 다른 판을 낸다.
# e8: 둔화가 이동만이 아니라 **행동 전체**를 늦춘다 (2026-09-10 결정). 사격형에게
#     효과가 없던 것이 고쳐지고, 몬스터가 거는 둔화도 함께 세진다.
# e9: 방벽이 **틱을 안 쓴다** — 규칙표는 켜고 끄는 것만 정하고 그 틱의 행동은
#     따로 돈다 (`plan.FREE_SKILLS`, 설계/5_스킬 §2.1).
# e10: 주문서가 넷으로 갈렸다 (2026-09-11). 순간이동·화염·부릅이 들어오고, 이미 걸린
#      주문서를 또 쓰는 규칙은 「불가」가 된다 — 예전 규칙표도 이 판정을 받으므로
#      같은 시드가 다른 판을 낸다.
# e11: 주문서가 **저절로 터진다** (2026-09-11 개정). 아이템마다 고정 트리거를 갖고,
#      조건이 맞으면 규칙 줄 없이 발동·소비된다 — 규칙표가 그 태그를 직접 쓰면 자동은
#      물러난다. 같은 규칙표가 같은 시드에서 다른 판을 낸다.
# e12: 적 선공이 **플레이어를 따라 옮긴다** (2026-09-14). 적 선공은 20~78 고정인데
#      플레이어는 `50 + 2×민첩` 으로 한계 없이 자라, 민첩 빌드는 5층부터, 레벨 20+ 은
#      전 층에서 **모든 적이 나보다 느렸다** — `적 선공 > 내 선공` 이 영영 거짓인 항이
#      된다. 이제 밴드가 보존되고, 그래서 턴 순서가 바뀐다 (`scaling.get_shifted_initiative`).
# e13: 마법 둘이 하는 일이 바뀌었다 (2026-09-17 요청). 연쇄 번개는 직선 대신 **적을
#      타고 튀고**(`CHAIN`), 서리 장판은 둔화 3틱 대신 **1틱 이동불가**(`ROOT`)를 넓게
#      건다. 새 상태 하나와 새 형태 하나가 들어왔으므로 같은 시드가 다른 판을 낸다 —
#      이 두 스킬을 안 쓰는 규칙표도 마찬가지다: `self_has_status` 의 목록이 늘어
#      인지 변수의 개수가 바뀌었다.
# e14: **독이 일을 하고, 인지 변수가 늘었다** (2026-09-17). `POISON` 은 목록에만 있고
#      아무도 안 걸던 상태였는데 유지 단계가 매 틱 3 을 깎기 시작했다 — 같은 시드가 다른
#      HP 를 낸다. 그리고 주술형 유형·셀렉터 둘·적 전용 마법 다섯이 들어오면서 **인지
#      스냅샷의 키 개수 자체가 바뀌었다**: 이 다섯을 안 쓰는 규칙표도 같은 판을 안 낸다.
ENGINE_VERSION = 14
