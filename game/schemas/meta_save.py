...
해금 블록, 층 도달 기록(규칙 슬롯 상한의 근거), 몬스터 도감, 코드 라이브러리(프리셋
8슬롯)를 담는다.

**장르 정정 (2026-08-30) 이후의 자리.** 이 파일은 "사망해도 남는 것만" 이라는 로그라이크
전제로 지어졌으나 이 게임은 **던전 RPG** 다 — 캐릭터가 이어지고 장비·아이템도 남는다.
그래서 "여기 없는 것은 사망 시 소멸" 이 더는 참이 아니다. 지금 이 파일의 자리는
**규칙 설계에 관한 것**(해금·도감·프리셋)이고, 아이템·화폐·레벨은 서버의 entity_record 와
item_instance 가 맡는다.
