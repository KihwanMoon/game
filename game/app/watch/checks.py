...
# 그림자가 이만큼 안 바뀌면 굳은 것이다. 봇 열이 1800초마다 돌고 깊은 죽음이 하루에
# 여러 건이므로, 반나절이면 몇 번은 바뀌었어야 한다. 실측으로 굳었을 때는 **이틀**이었다.
#
# **봇 리듬을 720 → 1800 으로 늦춘 뒤에도 12시간을 그대로 둔다** (2026-09-10). 판이
# 시간당 50 → 20 으로 줄었으니 여유가 좁아지긴 했지만, 12시간이면 여전히 240판이다 —
# 그래도 안 바뀌었으면 리듬이 아니라 규칙이 틀린 것이고, 그것이 이 검사가 보는 것이다.
