...
def list_floor_rooms(rooms: dict[str, RoomTemplate], floor: int) -> tuple[str, ...]:
    """그 층에서 나올 수 있는 방을 모은다.
    ...
    """
    return tuple(sorted(key for key, room in rooms.items() if room.min_floor <= floor))


def build_room_chain(
    rooms: dict[str, RoomTemplate], floor: int, first_room_id: str, length: int
) -> tuple[str, ...]:
    """이 런이 돌 방 목록을 만든다.
    ...
    **나머지는 겹치지 않게 고른다.** 후보가 모자라면 그때만 되풀이하되 **바로 앞 방과는
    ...
    """
    candidates = list_floor_rooms(rooms, floor)
    if not candidates:
