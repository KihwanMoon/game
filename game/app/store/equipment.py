...
def apply_equip(pool: ConnectionPool, account_id: int, item_id: int, slot: EquipSlot) -> None:
    ...
    with pool.connection() as connection:
        existing = connection.execute(
            "SELECT item_id FROM equipment_slot WHERE account_id = %s AND slot = %s",
            (account_id, str(slot)),
        ).fetchone()
    if existing is not None:
        apply_unequip(pool, account_id, slot)
    with pool.connection() as connection:
        # 인벤토리에서 빼고 슬롯에 넣는다. 한 트랜잭션이라 둘 중 하나만 되는 일이 없다.
        connection.execute(
            "DELETE FROM inventory_slot WHERE account_id = %s AND item_id = %s",
            (account_id, item_id),
        )
        connection.execute(
            "INSERT INTO equipment_slot (account_id, slot, item_id) VALUES (%s, %s, %s)",
            (account_id, str(slot), item_id),
        )
    record_item_event(pool, account_id, item_id, EVENT_EQUIP, str(slot))


def apply_unequip(pool: ConnectionPool, account_id: int, slot: EquipSlot) -> int | None:
    """슬롯을 비우고 인벤토리로 돌려보낸다.
    ...
    if row is None:
        return None
    item_id = int(row[0])
    index = find_empty_slot(pool, account_id)
    if index is None:
        raise ValueError("인벤토리가 가득 차 해제할 수 없다")
    with pool.connection() as connection:
        connection.execute(
            "DELETE FROM equipment_slot WHERE account_id = %s AND slot = %s",
            (account_id, str(slot)),
        )
        connection.execute(
            "INSERT INTO inventory_slot (account_id, slot_index, item_id) VALUES (%s, %s, %s)",
            (account_id, index, item_id),
        )
    record_item_event(pool, account_id, item_id, EVENT_UNEQUIP, str(slot))
    return item_id


def remove_item(pool: ConnectionPool, account_id: int, item_id: int) -> bool:
    ...
    with pool.connection() as connection:
        cursor = connection.execute(
            "DELETE FROM item_instance WHERE id = %s AND owner_account_id = %s",
            (item_id, account_id),
        )
        removed = cursor.rowcount == 1
    if removed:
        record_item_event(pool, account_id, None, EVENT_DISCARD, str(item_id))
    return removed


def mark_item_broken(pool: ConnectionPool, account_id: int, item_id: int) -> None:
    ...
    with pool.connection() as connection:
        connection.execute(
            "UPDATE item_instance SET is_broken = true WHERE id = %s AND owner_account_id = %s",
            (item_id, account_id),
        )
    record_item_event(pool, account_id, item_id, EVENT_BREAK)


def apply_repair(pool: ConnectionPool, account_id: int, item_id: int) -> int:
    """복구비용을 내고 파손을 푼다.
    ...
    with pool.connection() as connection:
        connection.execute(
            "UPDATE item_instance SET is_broken = false WHERE id = %s AND owner_account_id = %s",
            (item_id, account_id),
        )
    record_item_event(pool, account_id, item_id, EVENT_REPAIR, str(REPAIR_COST))
    return balance
