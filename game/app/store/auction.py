...
* **수수료가 푼을 태운다.** 등록할 때 떼며, 지금 이 게임에서 유일한 배출구다.
  없으면 푼이 단조 증가해 몇 주 만에 가격이 무의미해진다.
...
# 등록 수수료 (정수 퍼센트). **푼의 유일한 배출구다** — 없으면 푼이 단조 증가해
...
def apply_purchase(
    ...
    with pool.connection() as connection:
        ...
        # 자유 거래로 두면 같은 아이템을 A→B→A 로 돌려 계정 사이에 푼을 씻을 수 있다.
        connection.execute(
