...
# 이미 연 칸을 다시 찍는 값 — **활자다, 푼이 아니다** (신설 2026-09-15).
#
# **하나인 이유는 활자가 이미 희소하기 때문이다.** 활자는 내 둔갑이 남의 장에서 이겨야
# 들어오고, 그림자는 목숨 셋을 다 쓰면 물러난다 — 한 마리가 평생 버는 것이 몇 개다.
# 여기에 또 증가값을 붙이면 두 번째 재주조가 사실상 불가능해진다.
RECAST_COST = 1
