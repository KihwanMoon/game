...
# 독. **거는 것도 깎는 것도 없었다** (2026-09-17까지). 인지 변수 목록에는 처음부터 있어서
# 규칙표가 `내 상태이상[중독]` 을 물을 수 있었는데, 그 값이 참이 되는 길이 세계에 없었다 —
# 물어볼 수는 있고 답은 영영 거짓인 항이다.
#
# 이제 유지 단계가 매 틱 깎는다. 지형 피해(용암)와 **같은 문을 지난다**: 방어력 감쇠는
# 안 거치고(전투 수식은 `_apply_strike` 안에 있다) 방벽은 막는다(`apply_damage` 안에
# 있다). 그래서 방벽이 독을 반으로 줄이고, 그것이 방벽에 답이 하나 더 생겼다는 뜻이다.
STATUS_POISON = "POISON"

# 이동불가. **둔화와 다른 축이다** — 둔화는 행동 전체를 반으로 늦추고(`slow.py`), 이것은
# 이동만 통째로 막는다. 때리는 것도 시전도 그대로 된다.
#
# 가르는 이유는 값이 아니라 질문이다. 둔화는 「얼마나 느려지는가」를 묻고 이것은 「어디에
# 묶이는가」를 묻는다 — 서리 장판이 화력으로도 통제로도 값을 못 했던 자리가 거기다
# (설계/5_스킬 §10.10: SLOW 는 안 움직여도 때리는 사격형에게 아무 효과가 없었다).
# 붙은 돌격형에게도 마찬가지였고, 이동만 막으면 **도망치려는 쪽**에만 걸린다.
#
# 순간이동 주문서는 안 막는다. 묶였을 때 빠져나갈 길이 하나는 있어야 「묶는다」가
# 처형이 아니라 수가 된다.
STATUS_ROOT = "ROOT"
