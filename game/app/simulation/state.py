...
class Entity:
    ...
    # 시전 축 셋 (설계/5_스킬 §10.7). 개체마다 다르므로 스킬 카탈로그가 아니라 개체가
    # 갖는다 — `skill_power_pct` 와 같은 자리다.
    cast_lead_cut: int = 0
    steady_cast: int = 0
    blast_radius: int = 0
    # **반경을 넓히는 유물이 무는 대가다** (§10.7 의 「쿨 16 → 24」). 대가가 없으면
    # 확산의 핵이 상위 호환이 되고, 그러면 §10.7 이 금지한 바로 그것이 된다 —
    # 규칙표를 안 바꾸고 끼기만 하면 되는 유물.
    cast_cooldown_add: int = 0
    # **후경직으로 남은 틱** (2026-09-19). 재주가 발동한 뒤 이만큼 굳는다 —
    # `SkillDef.recover` 가 값을 주고 유지 단계가 매 틱 줄인다. 예고(`telegraph`)의 짝이다.
    recover_ticks: int = 0
