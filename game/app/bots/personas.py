...
# 빨리 돌면 전리품과 푼이 봇 쪽에서 나오고, 그러면 시장을 채우려던 것이 시장을 봇의
# 것으로 만든다.
#
# 「한 판」은 티켓 하나다. 하강 한 번이 층마다 청구를 여러 번 올리지만 그것은 한 판이며,
# 사람도 그렇게 논다.
#
# **5 → 2 로 낮췄다 (2026-09-10).** 열 명이 합쳐 시간당 50판이던 것이 20판이 된다.
# 봇은 세계를 혼자 두지 않으려고 있는 것이지 세계를 채우려고 있는 것이 아닌데, 위에
# 적은 두 이유(부하·경제)가 실제로 그 방향으로 기울고 있었다.
MAX_RUNS_PER_HOUR = 2
...
# 열 명. **전부 상한으로 돈다** — 봇마다 시간당 두 판이고 합쳐서 20판이다. 리듬을
