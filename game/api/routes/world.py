...
# 전적·물러난 둔갑을 몇 줄까지 실을까.
#
# **10 이 데이터를 잘라내고 있었다** (2026-09-17 실측). 한 계정이 236판을 치렀는데 화면은
# 열 줄만 받아서 나머지 226판이 **아예 안 보였다** — 찾기나 페이지를 붙여도 볼 것이
# 없었던 셈이다. 화면의 기본 한 장은 여전히 열 줄이고(`pageSize`), 「더 보기」로 나머지에
# 닿는다. 한 줄이 작아서 백 줄을 실어도 payload 가 가볍다.
BOUT_LIMIT = 100
