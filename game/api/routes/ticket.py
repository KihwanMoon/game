...
def create_run_ticket(request: TicketRequest, account: CurrentAccount) -> TicketResponse:
    ...
    return TicketResponse(
        **vars(ticket),
        monster_snapshot=[build_battle_payload(pool, item) for item in snapshots],
    )


def build_battle_payload(pool: ConnectionPool, item: MonsterSnapshot) -> dict:
    """전투 화면으로 갈 스냅샷 한 절 — **주인 이름을 덧붙인다**.

    **저장되는 절에는 안 붙인다** (`save_snapshots` 는 위에서 따로 부른다). 그쪽은 재시뮬이
    읽는 코어 계약이라, 표시용 값을 끼우면 같은 티켓이 다른 글자로 저장된다 (R5). 이름은
    판정에 아무 영향이 없으므로 응답에만 실린다.

    **그림자만 이름이 있다.** 여느 몬스터는 주인이 없다 — 빈 문자열이면 화면이 종 이름을
    그대로 쓴다.
    ...
    Returns:
        응답에 실을 절.
    """
    payload = build_snapshot_payload(item)
    if not check_is_doppel(item.kind_id):
        return payload
    return {**payload, "owner_name": read_doppel_owner_name(pool, item.record_id)}
