...
## 디스크 — 무엇이 자라는가 (2026-09-15)

이 스택은 `/` 에 산다. 그 디스크는 홈 캐시와 시스템이 이미 대부분을 먹고 있어서, **우리가
자라게 두면 DB 와 같은 칸을 먹는다** — 채워지면 게임이 아니라 저장이 먼저 멈춘다.

자라는 것이 셋이다.

| 무엇 | 왜 자라나 | 어떻게 막았나 |
|:--|:--|:--|
| 빌드 캐시 | 빌드할 때마다 쌓이고 안 지워진다 | `docker builder prune` — 2026-09-15 에 9.4GB 회수 |
| 컨테이너 로그 | json-file 기본값에 **회전이 없다.** 봇이 30초마다 긴 줄을 남긴다 | `x-logging` 앵커 (10MB × 3장 = 컨테이너당 30MB 상한) |
| `monster_snapshot` | 티켓 하나가 서른 몇 개를 남긴다. 두 주에 34만 개 | `scripts/prune_snapshots.py` — 184MB → 4.3MB |

**이미지는 안 지운다.** 20개 중 6개가 돌고 있고, 나머지는 되돌릴 때 쓴다.

**로그 상한은 컨테이너를 다시 만들어야 붙는다.** `docker compose up -d` 로 재생성하면
되고, 붙었는지는 이렇게 본다:

```bash
docker inspect -f '{{.HostConfig.LogConfig.Config}}' game-bots-1
# map[max-file:3 max-size:10m]
```

**`edge-proxy` 는 이 파일이 아니다.** 그쪽 로그가 가장 큰데(13MB) 저장소가 다르므로
(`/data/workspace/edge-proxy`) 상한도 거기서 걸어야 한다.
