...
def test_room_spawns_carry_the_floor_scale(balance, templates):
    ...
    shallow = build_engine(templates["open_field"], balance, seed=1, floor=FIRST_FLOOR)
    deep = build_engine(templates["open_field"], balance, seed=1, floor=DEEP_FLOOR)
...
def test_summoned_minions_carry_the_floor_scale(balance, templates):
    ...
    # 층이 깊어질수록 유리해진다.
    engine = build_engine(templates["pillars"], balance, seed=1, floor=DEEP_FLOOR)
...
def test_engine_floor_overrides_a_reused_tracker(balance, templates):
    ...
    build_engine(templates["open_field"], balance, seed=1, floor=DEEP_FLOOR, pressure=tracker)
