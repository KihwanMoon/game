"""봇에게 푼을 넘긴다 (2026-09-06).

**봇에게 밑천을 주는 자리다.** 봇이 경매에서 사려면 푼이 있어야 하는데, 벌이가 느린
봇은 영영 못 산다 — 그러면 「봇이 아무것도 안 산다」가 봇의 규칙이 아니라 잔액의 문제가
되고, 우리가 보려던 것(봇이 무엇을 고르는가)이 안 보인다.

지키는 것은 셋이다.

1. **푼을 만들지 않는다.** 주는 쪽에서 빠진 만큼만 들어간다 (결정 #02).
2. **봇에게만 간다.** 사람에게 열면 계정 사이 푼 이동이 되고, 봇 파밍으로 번 것을
   사람 계정에 모을 수 있다 (T11).
