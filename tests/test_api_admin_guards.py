...
def test_there_is_no_route_that_grants_admin(client):
    ...
    assert sorted(path for path in paths if "/admin/" in path) == [
        # 새로 들어온 계정을 본다 (U1, 2026-09-18). **읽기만 한다** — 이 문으로는
        # 아무것도 안 바뀐다. 익명 계정의 생성 시각과 가입 여부를 내므로 관리자
        # 문지기 뒤에 둔다: 「누가 언제 들어왔나」는 운영이 볼 것이지 공개할 것이 아니다.
        "/api/admin/arrivals",
        ...
        # 길이 된다.
        "/api/admin/bot/bag",
        # 봇에게 밑천을 준다 (2026-09-06). **푼을 만들지 않는다** — 주는 쪽에서 빠진
        # 만큼만 들어간다. 봇에게만 넘어가므로 계정 사이 푼 이동이 안 열린다.
        "/api/admin/bot/coin",
        ...
        "/api/admin/bot/gift",
        # 봇 이름의 **뒷자리만** 고친다 (U3, 2026-09-18). 접두어 `bot_` 은 서버가 지키고
        # 받지도 않는다 — 그것이 「이것이 봇이다」를 싣는 유일한 채널이라, 떼는 순간
        # 순위표·경매·도감 세 화면에서 봇이 사람과 구별되지 않는다 (2026-09-11 신고).
        # `nickname` 을 안 쓰는 이유도 같다: 표시 이름이 그것을 먼저 보므로 접두어가
        # 화면에서 밀려난다. 이름이 겹치면 거절하고, 바꾼 것은 감사 기록에 남는다.
        "/api/admin/bot/name",
        "/api/admin/bots",
        "/api/admin/catalog",
