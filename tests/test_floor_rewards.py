...
#
# **1장 방 풀이 여섯에서 넷으로 줄면서 다시 골랐다** (2026-09-16). 기둥 숲·네거리를
# 2장으로 올렸더니 시드 1 이 이겨 버렸다 — 위 주석이 적어 둔 「층 풀이 바뀌면 시드의
# 뜻도 바뀐다」가 그대로 일어난 것이고, **검사가 깨진 것이 곧 바뀜이 먹혔다는 증거**다.
# 새로 잰 값: 79 시드 중 68 이 이긴다(86%). 지는 시드 3 은 두 방을 깨고 세 번째에서 죽어
# 「죽은 방에서 멈춘다」를 그대로 보여 준다.
WINNING_SEED = 9
LOSING_SEED = 3
