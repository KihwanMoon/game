"""벤치마크 규칙표가 서로 다른 판을 만드는가 (R2, GDD §8.3).

러너가 도는지는 `test_chain_and_batch` 가 본다. 여기는 **그 위에서 무엇이 갈리는가**를
본다 — 규칙 순서가 결과를 바꾸는지, 사후 분석이 범인을 짚는지, 진 판의 기울기가 두 종류의
패배를 갈라내는지, 폴백 한 줄이 실제로 맞히는지.
