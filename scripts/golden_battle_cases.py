...
ADVANCED_CASES = (
    ...
    # **2막 후반의 두 굿을 층 1 에서 태운다** (2026-09-18). 제 방(16·20장)에서는 층
    # 스케일이 32배라 판이 네 틱에 끝나고, 그 안에서는 2틱 예고가 **한 번도 안 터진다** —
    # 실제로 `dirge_hollow` 골든에 `HEX_DIRGE` 가 0건이었다. 상태가 한 번도 안 걸리면
    # 그 경로는 두 코어가 갈려도 G3 가 침묵한다: `ROOT`·`POISON` 이 층 8·12 골든에서
    # 잡히는 것과 같은 이유로, 얕은 층에 한 판씩 둔다.
    #
    # 넋두리는 `SLOW` 를 거는 유일한 자리이고(홀짝으로 행동을 거르는 `slow.py`),
    # 조종은 `cancel_on_hit` 이 꺼진 유일한 자리다 — 압박 표가 붙어서 때리는 동안에도
    # 굿이 그대로 떨어져야 한다.
    ("g0_kite", "open_field", 6395, (("dirge_shaman", 6, 4),)),
    # **붙여 세운다.** 여섯 칸 밖에 두었더니 압박 표가 예고 3틱 동안 걸어오기만 해서,
    # 정작 「맞고도 안 끊긴다」가 한 번도 안 찍혔다. 두 칸이면 둘째 틱부터 때린다 —
    # 같은 자리에서 넋두리는 `예고 취소 — 피격` 이 찍히므로 둘이 대조가 된다.
    ("g0_pressure", "open_field", 6432, (("knell_shaman", 3, 4),)),
)
...
NEW_ROOM_CASES = (
    ...
    # **2막 후반 다섯** (2026-09-18). 여기서 처음 고정되는 것이 둘이다 — `SLOW` 가 매 틱
    # 행동을 거르는 홀짝 판정(`slow.check_slowed_this_tick`)과, `cancel_on_hit` 이 꺼진
    # 굿이 맞고도 안 끊기는 경로다. 둘 다 두 코어가 갈리면 이긴 판이 진 판으로 확정된다.
    ("g0_pressure", "dirge_hollow", 6210, 16),
    ("g0_kite", "calling_yard", 6247, 17),
    ("g0_cover", "knell_shrine", 6284, 18),
    ("g0_pressure", "mudang_row", 6321, 19),
