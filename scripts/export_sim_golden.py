...
* `cycle` — 그 빈자리를 메우는 **대조 전용** 정책이다. 틱과 엔티티 id 만 보고 행동 13개를
...
# 동결된 행동 13개 전부. 순서를 바꾸면 기준 로그가 통째로 달라진다.
ACTION_CYCLE = (
    "APPROACH",
    "ATTACK",
    "SKILL_2",
...
# 템플릿 id · 시드. 템플릿마다 적 구성과 지형이 달라 서로 다른 코드 경로를 밟는다.
FALLBACK_CASES = (
    ("open_field", 1),
    ("corridor", 12345),
    ("pillars", 7),
...
CYCLE_CASES = (
    ("open_field", 4242, (("bomb_slime", 5, 4), ("mender_acolyte", 6, 2))),
    ("pillars", 555, (("arch_summoner", 7, 4), ("veteran_rusher", 4, 6))),
    ("hazard_field", 808, (("longbow_archer", 6, 4),)),
    ("spring_bait", 31337, (("bomb_slime", 6, 6),)),
)
...
class CyclingPolicy:
    ...
    def plan_action(
        ...
        target = resolve_target(SELECTOR_NEAREST, entity, state, self._kind_types)
...
def build_battle_cases() -> list[dict[str, Any]]:
    ...
    cases = [
        build_case(balance, templates, template_id, seed, POLICY_FALLBACK)
        for template_id, seed in FALLBACK_CASES
    ]
    cases.extend(
        build_case(balance, templates, template_id, seed, POLICY_CYCLE, extras)
        for template_id, seed, extras in CYCLE_CASES
    )
    return cases
...
def build_golden_document() -> dict[str, Any]:
    ...
    return {
        "_comment": [
            ...
            "policy=cycle 은 대조 전용 결정기다 — 행동 13개를 틱마다 차례로 돌린다.",
        ],
        "max_ticks": MAX_TICKS,
        "floor": FLOOR,
