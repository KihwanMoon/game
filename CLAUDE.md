...
`docs/README.md` 가 지도다. 전체를 한 번에 보려면 `docs/13_통합_기획서.md` →
`docs/14_통합_시스템설계.md`, 다음에 무엇을 정해야 하는지는
**`docs/11_결정대기_목록.md`** (미결 45건, 「언제 정해야 싼가」로 분류).

`13`·`14`·`15` 는 **통합 뷰이지 정본이 아니다.** 시스템별 정본은 `07`(저장·멀티플레이)·
`09`(아이템)·`10`(스킬)·`12`(몬스터)이고, 기획·기술 정본은 `01`·`02` 다.
