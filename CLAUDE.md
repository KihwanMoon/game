...
| 폴더 | 무엇 | 시작점 |
|:--|:--|:--|
| `docs/기획/` | 무엇을 만드는가 | `1_통합기획서` (세계관은 `4_세계관`) |
...
변형했다 — §12 가 허용하는 범위다. 계층 경계 원칙은 그대로 지켰다.

`ls` 가 말해 주지 않는 것만 적는다.

- `game/main.py` 가 컴포지션 루트다. 코어를 조립하는 유일한 지점이다.
- `game/schemas/` 는 I/O 계약(TDD §3)이며 **TS 로 이식되는 유일한 코드 자산**이다.
- `game/app/core/` 는 도메인을 모른다 — RNG·이벤트 로그·에러 규격만 든다.
- `game/app/store/` 는 서버 저장소(PostgreSQL)이고 **코어는 이것을 모른다**.
- `game/app/services/` 는 유스케이스다 — 파일 하나가 시나리오 하나다.
- `game/api/` 는 검증 서버(FastAPI)다. **얇게 유지한다** — 판정은 `services/`,
  저장은 `app/store/` 가 한다.
...
`frontend/src/core/` 는 **파이썬 코어를 그대로 이식한 것**이고, 두 코어가 같은 시드에서
비트 단위로 같은 결과를 내야 한다(게이트 G3). 이식 규약과 TS 쪽 불변 조건은
`frontend/CLAUDE.md`, 상세는 `frontend/README.md` 에 있다.

**굽는 화면은 둘이다** — `/` 가 제품 화면(규칙 에디터 ↔ 전투 관전 ↔ 사후 분석)이고
`/admin.html` 이 관리 화면이다. `/ds.html`·`/battle.html`·`/hud.html` 은 부품·렌더러·
되감기 확인용이며 **개발 서버에서만 열린다** — 공개 도메인에 서 있던 것을 2026-09-16 에
산출물에서 뺐다. 막는 규칙 대신 안 굽는 쪽을 골랐다: 규칙은 풀 수 있지만 없는 파일은
못 연다.
