...
| `docs/결정/` | 아직 안 정한 것 | **`1_결정대기목록`** — 미결 48건 |
...
정본이 **둘로 나뉜다.** 프로젝트 두 개를 헷갈리면 토큰을 엉뚱한 곳에 올리게 된다.

| 무엇 | 프로젝트 | 타입 |
|:--|:--|:--|
| **토큰·styles** | `7a323244-94a4-426b-a3b5-1bb1c949c195` "Design System" | 디자인 시스템 |
| **화면 시트** (`.dc.html`) | `fae25530-140a-4873-b9f9-684645b541c6` "game" | 일반 프로젝트 |

`design/` 은 **토큰 정본(`7a323244`)의 사본**이다. 고칠 일이 생기면 사본이 아니라 정본을
고치고 다시 가져온다 — 또는 `DesignSync` 로 역이관한다(2026-08-30 에 `spacing.css` 를
그렇게 되돌렸다).

**`fae25530` 은 디자인 시스템이 아니다** (`PROJECT_TYPE_PROJECT`). 타입은 생성 시
고정이라 거기에 토큰을 올려도 디자인 시스템이 되지 않는다. 그쪽은 게임 화면 시트가
사는 곳이고, `_ds/` 아래에 `7a323244` 를 참조로 끌어다 쓴다.

상세와 컴포넌트 계약은 `design/README.md` 참조.
...
계정은 **익명으로 시작해 가입으로 승격된다.** 자격증명을 계정 행의 빈 컬럼에 채우므로
계정 id 가 바뀌지 않고 세이브·티켓·제출이 전부 따라온다.

**한 계정은 한 기기다** (개정 2026-09-01). 로그인이 그 계정의 다른 기기 토큰을 지우고,
튕긴 기기는 401 을 받는다. 예전에는 반대였는데 — 토큰을 하나 더 붙여 두 기기를 함께 쓸 수
있었다 — 같은 계정의 상태가 두 벌 돌면서 **나중에 저장한 쪽이 앞의 것을 덮었다.** 쓰는
사람에게 그것은 "규칙이 사라졌다" 로 보인다.

같은 이유로 **로그인은 서버가 이긴다.** 로그인은 "이 기기를 그 계정으로 만든다" 는 명시적
행동이므로 초안·슬롯을 서버 것으로 갈아 끼운다. 그냥 새로고침(mount)은 반대다 — 이 기기에
있던 것을 지킨다. 로그아웃은 서버 토큰과 **이 기기의 저장을 함께** 지운다.
