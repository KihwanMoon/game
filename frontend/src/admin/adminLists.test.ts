 ...
 *
 * **정의 목록(`<dl>`)도 안 센다** (2026-09-19). 이 검사가 지키는 것은 「찾기와 빈 경우가
 * 화면마다 갈리는 것」인데, 이름과 설명을 짝지어 적는 블록은 본래 찾기도 빈 경우도
 * 없다 — 재주 표가 칸마다 「왜 이 항목뿐인가」를 적는 자리가 그것이다. 그런 글을 `<ul>`
 * 로 적으면 여기 걸리는데, 걸릴 짓이라서가 아니라 **마크업이 틀린 것**이다.
 *
 * @param source 파일 내용.
 * @returns 걸린 자리 수.
 */
