 ...
 * 특정된다 (GDD §8.2, P1).
 *
 * 값의 출처는 **직전 틱의 평가**다. 카드 헤더 오른쪽의 `평가값 = 직전 틱` 이 그 사실을
 ...
 *
 * **「이번 틱」이라 적지 않는다.** 편집 중에는 틱이 돌지 않으므로 그 말은 지금 값처럼
 * 읽히는데, 여기 있는 수는 직전 틱이 남긴 것이다 — 이 줄이 피하려던 오해가 바로
 * 그것이다. prop 문서도 전부 「직전 틱」으로 적는다 (`TermReadings`).
 */
export const MEASURE_SOURCE = '평가값 = 직전 틱'
