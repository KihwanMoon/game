 ...
 */
export type SheetTab = 'ads' | 'vitals' | 'rules' | 'log' | 'reward'
 ...
 */
export const SHEET_TABS: readonly SheetTab[] = ['ads', 'vitals', 'rules', 'log', 'reward']

/** 탭 라벨. 카운트는 따로 붙인다. */
export const SHEET_TAB_LABELS: ReadonlyMap<SheetTab, string> = new Map([
  // **첫 자리이되 기본 선택은 아니다** (2026-09-17 요청). 순서는 맨 앞이지만 판이
  // 시작할 때 열리는 탭은 `INITIAL_TAB`(상태) 그대로다 — 광고가 먼저 열리면 그것이
  // 「내용을 가로막는 배치」가 되고, 구글 정책이 그것을 막는다.
  ['ads', '광고'],
  ['vitals', '상태'],
  ['rules', '내력'],
  ['log', '실행 로그'],
